...
// Initialize App
export async function initializeRealtimeDatabase( firebaseConfig ) {
    const firebaseApp = initializeApp(firebaseConfig);
    const auth = getAuth( firebaseApp );
    const db = getDatabase( firebaseApp );

    let firebaseUserId = 'not initialized yet';
    ...
    return [ db, firebaseUserId ];
}


export async function initializeSecondRealtimeDatabase( firebaseConfig ) {
  const firebaseApp2 = initializeApp(firebaseConfig , 'firebaseApp2');
  const auth = getAuth( firebaseApp2 );
...
//------------------------------------------------------
export async function writeRealtimeDatabase( db, myPath , value ) {
...
export async function readRealtimeDatabase( db, myPath ) {
      var result = [];
...
export async function writeURLParameters( db, myPath ) {
        ...
        if (paramString) {
            ...
            for(let i = 0; i < params_arr.length; i++) {
                ...
                await writeRealtimeDatabase( db, pathNow , value )
            }
        }
...
export async function blockRandomization( db, studyId , condition , numConditions , maxCompletionTimeMinutes , numDraws ) {
    ...
    await writeRealtimeDatabase( db, timePath  , serverTimestamp() );

    const serverTimeNow = await readRealtimeDatabase( db, timePath );
    ...
    try {
      ...
      const result = await readRealtimeDatabase( db, condPath );
    ...
    await writeRealtimeDatabase( db, condPath+'/' + key + '/'  , condRecord );
...
export async function finalizeBlockRandomization( db, studyId , condition ) {
     const key = randomId;
     const condPath =  studyId+'/studyInfo/conditionCounts/'+condition;
     await writeRealtimeDatabase( db, condPath+'/' + key + '/hasStarted'  , 0 );
     await writeRealtimeDatabase( db, condPath+'/' + key + '/hasCompleted'  , 1 );
}


export async function blockRandomization_readonly( db, studyId , condition , numConditions , maxCompletionTimeMinutes , numDraws ) {
  if (numDraws > numConditions) {
        throw( "Number of draws exceeds number of available conditions -- split up the draws");
  }
